...
describe('admin batch voice audio targets', () => {
  ...
  it('derives the 368 Admin Voice targets from the 368-target manifest', () => {
    ...
    expect(completeTargets).toHaveLength(368)
    expect(completeTargets.some((target) => target.moduleType === 'pronunciation')).toBe(false)
    expect(visibleTargets).toHaveLength(368)
