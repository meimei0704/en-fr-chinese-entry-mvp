...
describe('course content', () => {
  it('exposes twelve lessons in canonical journey order with static audio paths', async () => {
    ...
    const expandedCounts: Record<string, Record<string, number>> = {
      ...
      'order-food': { dialogue: 14, patterns: 6, vocab: 10, practice: 2, cards: 6 },
      'phone-and-payment': { dialogue: 10, patterns: 5, vocab: 11, practice: 2, cards: 6 },
      ...
      'convenience-store-run': { dialogue: 14, patterns: 4, vocab: 12, practice: 2, cards: 6 },
      'ask-for-help-problem': { dialogue: 16, patterns: 2, vocab: 10, practice: 2, cards: 6 },
      'pharmacy-help': { dialogue: 13, patterns: 4, vocab: 11, practice: 2, cards: 6 },
      'small-talk': { dialogue: 14, patterns: 4, vocab: 7, practice: 2, cards: 6 },
    }
  ...
  it('keeps the new small-talk lesson as the final lesson', async () => {
    ...
    expect(lesson.dialogue.lines.map((line) => line.id)).toEqual(
      Array.from({ length: 14 }, (_, index) => `small-talk-line-${String(index + 1).padStart(2, '0')}`),
    )
  })
  ...
  it('ships non-empty MP3 audio files for every Chinese playback reference', async () => {
    ...
    expect(audioPaths).toHaveLength(547)
    expect(new Set(audioPaths).size).toBe(547)
