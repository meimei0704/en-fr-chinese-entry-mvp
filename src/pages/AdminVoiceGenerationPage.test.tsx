...
describe('AdminVoiceGenerationPage', () => {
  ...
  it('loads all course snapshots and shows the 403 visible zh-CN audio targets', async () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /403 audio targets/i })).toBeVisible()
  ...
  it('shows 403 targets from the five visible modules while retaining generic pronunciation copy', async () => {
    installBatchFetchMock()

    renderRoute('/admin/voice')

    expect(await screen.findByRole('heading', { level: 2, name: /403 audio targets/i })).toBeVisible()
    const metrics = within(screen.getByTestId('admin-voice-metrics'))
    const targetGrid = screen.getByTestId('admin-voice-target-grid')
    expect(metrics.getByText('403', { selector: 'strong' })).toBeVisible()
  ...
  it('shows recorder guidance and gates microphone capture behind consent', async () => {
    ...
    await screen.findByRole('heading', { level: 2, name: /403 audio targets/i })
  ...
  it(
    ...
    expect((await screen.findAllByText(/403 generated/i))[0]).toBeVisible()
    expect(screen.getByRole('button', { name: /apply approved to drafts/i })).toBeDisabled()
    ...
    expect(generateBodies).toHaveLength(403)
  ...
  it('marks provider failures per row without changing original audio targets', async () => {
    ...
    expect((await screen.findAllByText(/403 failed/i))[0]).toBeVisible()
