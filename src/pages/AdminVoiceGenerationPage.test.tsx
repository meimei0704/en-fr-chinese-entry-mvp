...
describe('AdminVoiceGenerationPage', () => {
  ...
  it('loads all course snapshots and shows the 478 visible zh-CN audio targets', async () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /478 audio targets/i })).toBeVisible()
  ...
  it('shows 478 targets from the five visible modules while retaining generic pronunciation copy', async () => {
    installBatchFetchMock()

    renderRoute('/admin/voice')

    expect(await screen.findByRole('heading', { level: 2, name: /478 audio targets/i })).toBeVisible()
    const metrics = within(screen.getByTestId('admin-voice-metrics'))
    const targetGrid = screen.getByTestId('admin-voice-target-grid')
    expect(metrics.getByText('478', { selector: 'strong' })).toBeVisible()
  ...
  it('shows recorder guidance and gates microphone capture behind consent', async () => {
    ...
    await screen.findByRole('heading', { level: 2, name: /478 audio targets/i })
  ...
  it(
    ...
    expect((await screen.findAllByText(/478 generated/i))[0]).toBeVisible()
    expect(screen.getByRole('button', { name: /apply approved to drafts/i })).toBeDisabled()
    ...
    expect(generateBodies).toHaveLength(478)
  ...
  it('marks provider failures per row without changing original audio targets', async () => {
    ...
    expect((await screen.findAllByText(/478 failed/i))[0]).toBeVisible()
