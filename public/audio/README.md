...
**Contract**: the 23 `reference-initial-*.mp3` files MUST play the initial's own call sound (呼读音), never the example word; card example hanzi (八/跑/哥 …) are display-only and must not be read into these files. Their live sha256 is locked by `public/audio/pinyin/reference-initial.sha256` and enforced by `src/content/pinyin/course.test.ts` (editing any file without updating the manifest turns the test red).
