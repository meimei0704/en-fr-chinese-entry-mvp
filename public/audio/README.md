...
- The five `reference-tone-[1-4].mp3` and `reference-tone-neutral.mp3` files MUST play the approved `mā/má/mǎ/mà/ma` recordings respectively (neutral is the independent `ma` recording, NOT a reuse of `mā`). Their live sha256 is locked by `public/audio/pinyin/reference-tone.sha256`.
...
**Cache busting**: these audio files are served with `Cache-Control: public, max-age=86400`, so browsers hold them for 24 h. Because the tone paths are stable, the five tone URLs in `src/content/pinyin/lesson1.ts` carry a `?v=<8-char sha256 prefix>` query param that is read straight through the pinyin course JSON (`pkg/pinyincontent/data/pinyin_course.json`, regenerated by `npm run content:pinyin:check`). `src/content/pinyin/course.test.ts` locks each tone URL's `v` to the matching manifest hash prefix, so the version can never go stale: **if the tone audio files change, regenerate the sha256 manifest, update the five `?v=` prefixes, and run `content:pinyin:check`**.
