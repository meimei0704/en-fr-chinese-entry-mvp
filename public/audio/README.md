...
- On 2026-08-17, `pharmacy-help/vocab-06.mp3` (肚子疼) was regenerated using the homophone 杜 (dù, unambiguous fourth tone) because the default TTS voice reads 肚子 as dǔzi (third-tone tripe reading) when the word stands alone. The dialogue and pattern copies (`line-06.mp3`, `pattern-02.mp3`, `line-07.mp3`) already read dù correctly in context and were unchanged.
...
- The four `reference-tone-[1-4].mp3` files and matching `tone-game-ma-[1-4].mp3` prompts MUST use the same approved `mā/má/mǎ/mà` files; `reference-tone-neutral.mp3` MUST reuse the approved first-tone `mā` file. Their live sha256 is locked by `public/audio/pinyin/reference-tone.sha256`.
