...
- On 2026-08-12, the four reviewed `mā/má/mǎ/mà` files replaced both the matching Tone reference cards and the four `ma` tone-game prompts. The Neutral tone reference intentionally reuses the approved first-tone `mā` recording because it was the closest accepted pronunciation. Existing paths remain unchanged.
- On 2026-08-14, the five Tone reference recordings were replaced with the uploaded `mā/má/mǎ/mà/ma` files (mā→`reference-tone-1`, má→`reference-tone-2`, mǎ→`reference-tone-3`, mà→`reference-tone-4`, ma→`reference-tone-neutral`). The Neutral reference now uses the independent `ma` recording. The `tone-game-ma-[1-4].mp3` prompts were NOT changed in this update. Existing paths remain unchanged.
...
- The five `reference-tone-[1-4].mp3` and `reference-tone-neutral.mp3` files MUST play the approved `mā/má/mǎ/mà/ma` recordings respectively (neutral is the independent `ma` recording, NOT a reuse of `mā`). The `tone-game-ma-[1-4].mp3` prompts are separate and not bound by this contract. Their live sha256 is locked by `public/audio/pinyin/reference-tone.sha256`.
