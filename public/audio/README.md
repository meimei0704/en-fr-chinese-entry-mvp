...
- Rate: `-8%`.
- Coverage: all current `DialogueLine.audio`, `SentencePattern.audio`, `VocabularyItem.audio`, `PronunciationTip.audio`, `PracticePrompt.audio`, and `ShortInputPrompt.audio` paths under `src/content/lessons/*.ts` (182 files after the Lesson 1 Arrival sample refresh; 90 files belong to the five new lessons).
